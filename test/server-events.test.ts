...
describe('Server Lifecycle Events', () => {
  ...
  describe('Server Registration Events', () => {
    test('should emit serverRegistered event when server is created', async () => {
      const mockFactory = mock.fn(async () => {
        const mcp = new McpServer({ name: 'math-server', version: '2.1.0' });
        return { server: mcp.server, name: 'math-server', version: '2.1.0' };
      });
    ...
    test('should emit serverUpdated event with old and new server info', async () => {
      const oldFactory = async () => {
        const mcp = new McpServer({ name: 'old-server', version: '1.0.0' });
        return { server: mcp.server, name: 'old-server', version: '1.0.0' };
      };
      const newFactory = mock.fn(async () => {
        const mcp = new McpServer({ name: 'new-server', version: '2.0.0' });
        return { server: mcp.server, name: 'new-server', version: '2.0.0' };
      });
  ...
  describe('Event Data Validation', () => {
    test('should provide complete serverRegistered event data', async () => {
      const mockFactory = async () => {
        const mcp = new McpServer({ name: 'analytics-server', version: '3.2.1' });
        return { server: mcp.server, name: 'analytics-server', version: '3.2.1' };
      };
    ...
    test('should provide complete serverUpdated event data', async () => {
      const oldFactory = async () => {
        ...
        return { server: mcp.server, name: 'weather-server', version: '1.0.0' };
      };
      const newFactory = async () => {
        const mcp = new McpServer({ name: 'weather-pro-server', version: '2.0.0' });
        return { server: mcp.server, name: 'weather-pro-server', version: '2.0.0' };
      };
